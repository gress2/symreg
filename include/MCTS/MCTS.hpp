...
namespace symreg 
{
  ...
  template <class MultiArmedBandit = decltype(UCB1)>
  class MCTS {
    private:
    ...
    public:
      MCTS(const MultiArmedBandit& = UCB1);
  ...
  /**
     ...
     */
    template <class MultiArmedBandit>
    MCTS<MultiArmedBandit>::MCTS(const MultiArmedBandit& mab)
     ...
     */
    template <class MultiArmedBandit>
    void MCTS<MultiArmedBandit>::simulate() {
     ...
     */
    template <class MultiArmedBandit>
    void MCTS<MultiArmedBandit>::iterate(std::size_t n) {
     ...
     */
    template <class MultiArmedBandit>
    bool MCTS<MultiArmedBandit>::make_move() {
     ...
     */
    template <class MultiArmedBandit>
    search_node* MCTS<MultiArmedBandit>::max_score_child(search_node* node) {
     ...
     */
    template <class MultiArmedBandit>
    search_node* MCTS<MultiArmedBandit>::choose_leaf() {
     ...
     */
    template <class MultiArmedBandit>
    void MCTS<MultiArmedBandit>::backprop(double value, search_node* curr) {
      while (curr) {
     ...
     */
    template <class MultiArmedBandit>
    std::vector<search_node*> MCTS<MultiArmedBandit>::get_up_link_targets(search_node* curr) {
     ...
     */
    template <class MultiArmedBandit>
    search_node* MCTS<MultiArmedBandit>::get_earliest_up_link_target(search_node* curr) {
     ...
     */ 
    template <class MultiArmedBandit>
    search_node* MCTS<MultiArmedBandit>::get_random_up_link_target(search_node* curr) {
      auto targets = get_up_link_targets(curr);
      if (targets.empty()) {
        return nullptr;
      }
     ...
     */
    template <class MultiArmedBandit>
    std::unique_ptr<brick::AST::node> MCTS<MultiArmedBandit>::get_random_action(int max_arity) {
     ...
     */
    template <class MultiArmedBandit>
    std::vector<std::unique_ptr<brick::AST::node>> MCTS<MultiArmedBandit>::get_action_set(int max_action_arity) {
     ...
     */
    template <class MultiArmedBandit>
    bool MCTS<MultiArmedBandit>::add_actions(search_node* curr) {
     ...
     */
    template <class MultiArmedBandit>
    std::shared_ptr<AST> MCTS<MultiArmedBandit>::build_ast_upward(search_node* bottom) {
     ...
     */
    template <class MultiArmedBandit>
    double MCTS<MultiArmedBandit>::rollout(search_node* curr) {
     ...
     */
    template <class MultiArmedBandit>
    std::string MCTS<MultiArmedBandit>::to_gv() const {
      std::stringstream ss;
      ss << "digraph {" << std::endl;
      ss << root_.to_gv();
      ss << "}" << std::endl;
     ...
     */
    template <class MultiArmedBandit>
    int MCTS<MultiArmedBandit>::get_random(int lower, int upper) {
     ...
     */
    template <class MultiArmedBandit>
    std::unordered_map<std::string, double>& MCTS<MultiArmedBandit>::symbol_table() {
     ...
     */
    template <class MultiArmedBandit>
    std::shared_ptr<AST> MCTS<MultiArmedBandit>::build_result() {
